...
# Download nuImages datset 

Under All, download Samples and Metadata from [nuImages dataset](https://www.nuscenes.org/nuimages#download).

Should have files `nuimages-v1.0-all-samples` and `nuimages-v1.0-all-metadata`
...
nuim = NuImages(dataroot='data/sets/nuimages', version='v1.0-train', verbose=True, lazy=True)

sample_idx = 2
sample = nuim.sample[sample_idx]
key_camera_token = sample['key_camera_token']
sample_data = nuim.get('sample_data', key_camera_token)
img_path = os.path.join('data/sets/nuimages', sample_data['filename'])
...
# Collecting the segmentation dataset

If running on Colab terminal, run

```
python collect_dataset.py --data train --dataroot "data/sets/nuimages" --type "front"
```

Will create dataset in the form:
...
`${ROOT_FOLDER} = /content/{train, val, test}`

A ZIP file (i.e. {train, val, test}.zip) of the dataset will be saved in `"/content/drive/MyDrive/VLR/project"` (folder will be created)

# Download segmentation dataset

Download: [nuImages segmentation dataset](https://drive.google.com/drive/folders/1qV4G34PEElAhbfiRtxpvUUZ85zvwbPOh?usp=sharing).

# Samples

| RGB | Segmentation  | 
| :---   |  :----:    | 
| ![Grid](images/000039.jpg) | ![Grid](images/000039_seg.jpg) |

# Category name to Class ID mapping

Refer `name_to_index.json`
